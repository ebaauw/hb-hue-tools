...
class EventStreamClient extends EventEmitter {
  ...
  listen () {
    this.request = https.request(this.options.url + this.options.resource, {
      ...
      checkServerIdentity: (host, cert) => {
        return this.options.client.checkCertificate(cert)
      },
      family: 4,
      headers: {
  ...
  parseUpdate (obj) {
    for (const data of obj.data) {
      ...
      const resource2 = ['', data.type, data.id].join('/')
      const attr = {}
      const state = {}
      const config = {}
      const attr2 = {}
      ...
      if (resource != null) {
        if (Object.keys(attr).length > 0) {
          /** Emitted when an `update` notification has been received
            ...
            * @param {string} resource - The changed API v1 resource.<br>
      ...
      if (resource2 != null) {
        if (Object.keys(attr2).length > 0) {
          /** Emitted when an `update` notification has been received
            ...
            */
          this.emit('changed2', resource2, attr2)
          emitted = true
        }
      }
      if (!emitted) {
