...
class HueDiscovery extends EventEmitter {
  ...
  async config (host) {
    ...
    const client = new HttpClient({
      ...
      validStatusCodes: https ? [200, 301] : [200]
    })
    client
      .on('error', (error) => {
  ...
  async _mdns () {
    ...
    browser4.on('up', (obj) => {
      this._found('mdns', obj.txt.bridgeid.toUpperCase(), obj.referer.address + ':443')
    })
  ...
  async _upnp () {
    ...
    upnpClient
      .on('error', (error) => { this.emit('error', error) })
      .on('searching', (host) => {
        /** Emitted when local search has started.
          *
    ...
    /** Emitted when local search has concluded.
      *
    ...
    try {
      ...
      if (Array.isArray(body)) {
        for (const bridge of body) {
          this._found(
            options.name, bridge.id.toUpperCase(),
