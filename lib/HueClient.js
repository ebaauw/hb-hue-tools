...
class HueClient extends HttpClient {
  ...
  static isHueBridge (config) {
    return ['BSB001', 'BSB002'].includes(config.modelid) &&
...
-----END CERTIFICATE-----`
  ...
  /** Create a new instance of a HueClient.
    ...
    * If no API key is known {@link HueClient#createApiKey createApiKey()} can
    ...
    * {@link HueClient#createApiKey createApiKey()}.
  ...
  constructor (params = {}) {
    ...
    if (_options.https && !_options.forceHttp) {
      options.https = true
      options.ca = HueClient.rootCertificate
      options.checkServerIdentity = (host, cert) => {
        return this.checkCertificate(cert)
      }
  ...
  /** Create an API key and set {@link HueClient#apiKey apiKey}.
    ...
    *
    * Before calling `createApiKey`, the link button on the Hue bridge must be
    * pressed.
    * @return {string} apiKey - The newly created API key.
    * @throws {HueError} In case of error.
    */
  async createApiKey (application) {
